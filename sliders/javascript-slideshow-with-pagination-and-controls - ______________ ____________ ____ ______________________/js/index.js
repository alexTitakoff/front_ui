(function() {
	...
	Slideshow.prototype = {
		init: function() {
			// обознначаем элементы
			this.wrapper = this.el.querySelector( ".slider-wrapper" );
			this.slides = this.el.querySelectorAll( ".slide" );
			this.previous = this.el.querySelector( ".slider-previous" );
			this.next = this.el.querySelector( ".slider-next" );
			this.navigationLinks = this.el.querySelectorAll( ".slider-pagination a" );
			this.index = 0; // данный индекс
			this.total = this.slides.length; // общее количество слайдеров
		...
		_slideTo: function( slide ) {
			...
			for( var i = 0; i < this.slides.length; i++ ) {  //индексируем слайды
